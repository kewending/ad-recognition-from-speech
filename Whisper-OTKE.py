...
from util.OTKE import OTKernel
from util.LOSS import FocalLoss, CrossModalContrastiveLoss, LabelSmoothingLoss

import os
model_name = "Whisper-OTKE"
...
class AudioTextClassifier(nn.Module):
    def __init__(self, num_labels=2, whisper_model="openai/whisper-small",  dropout=0.1):
        super().__init__()
        self.whisper = AutoModel.from_pretrained(whisper_model).encoder
        self.embed_dim = self.whisper.config.hidden_size
    ...
    def forward(self, audio_input_features, labels=None):
...
class MultimodalTrainer(Trainer):
    def __init__(self, train_dataset=None, alpha=None, gamma=2.0, seed=42, label_smoothing=0.1, num_classes=2, *args, **kwargs):
        ...
        self.CrossModalContrastiveLoss = CrossModalContrastiveLoss()
        self.seed = seed
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        outputs = model(audio_input_features)
...
def objective(seed):
    ...
    print(f"Vallidation Result with seed {seed}: {val_results}")

    # Evaluate on the test set
    test_results = trainer.evaluate(AD_dataset_encoded["test"])
    print(f"Test Result with seed {seed}: {test_results}")
...
print(f"Average test results across all runs: {avg_test_results}")

std_test_results = {key: np.std([res[key] for res in test_results_list]) for key in test_results_list[0].keys()}
print(f"Std test results across all runs: {std_test_results}")